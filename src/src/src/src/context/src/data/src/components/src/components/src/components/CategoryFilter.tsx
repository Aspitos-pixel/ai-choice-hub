...
export function CategoryFilter({ selectedCategory, setSelectedCategory }: CategoryFilterProps) {
  ...
  const pillStyle = (active: boolean): React.CSSProperties => ({
    ...
    fontWeight: 600, fontSize: '.9rem', cursor: 'pointer', transition: 'all 0.2s', whiteSpace: 'nowrap' as const,
  });

  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: '2.5rem' }}>
      <button style={pillStyle(selectedCategory === 'all')} onClick={() => setSelectedCategory('all')}>{t('filter.all')}</button>
      {CATEGORIES.map(cat => (
        <button key={cat.id} style={pillStyle(selectedCategory === cat.id)} onClick={() => setSelectedCategory(cat.id)}>
          {t(cat.labelKey)}
        </button>
      ))}
    </div>
